...
> **核心理念**：Haiku / Sonnet 跑主迴圈當執行者，Opus 退居幕後當「顧問」。
>
> 基於 [AgentOpt 論文](https://arxiv.org/html/2604.06296v1)、[Anthropic 官方 Advisor Strategy](https://claude.com/blog/the-advisor-strategy) 整理。
...
| 角色 | 模型 | 職責 | 特性 |
|------|------|------|------|
| **執行者（Executor）** | Haiku / Sonnet | 驅動主迴圈、呼叫工具、讀寫檔案、產出結果 | 快速、便宜、忠實遵循流程 |
| **顧問（Advisor）** | Opus | 提供高階策略、架構決策、邊界案例判斷 | 按需出場、僅回傳建議 |

---

## 官方 API 實作

Anthropic 官方提供了 `advisor_20260301` 工具類型：
...
| 參數 | 說明 |
|------|------|
| `model`（外層） | 執行者模型（`claude-sonnet-4-6` 或 `claude-haiku-4-5`） |
| `type` | 固定為 `advisor_20260301` |
| `model`（工具內） | 顧問模型（`claude-opus-4-6`） |
| `max_uses` | 每次 API 請求中，執行者最多可諮詢顧問的次數 |
