...
show_skill "spec-interview"   "需求訪談、規格書"                  "requirements interview, spec"
...
[ "$ondemand_count" -eq 0 ] && echo -e "  ${DIM}（目前無按需規則，4 個規則皆為自動載入）${RESET}"
echo ""
...
echo -e "${DIM}────────────────────────────────────────────────────────────────${RESET}"
echo -e "  ${DIM}docs/INDEX.md          ·  進階文件索引（參考用）${RESET}"
echo -e "  ${DIM}.claude/REFERENCES.md  ·  官方文件對照表（參考用）${RESET}"
echo -e "  ${DIM}prompts.md             ·  萬用 Prompt 模板（參考用）${RESET}"
